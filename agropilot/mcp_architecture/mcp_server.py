...
The agent dynamically selects which tools to call.
"""
...
mcp = FastMCP("AgriPilot MCP Server")
...
# The @mcp.tool() decorator is explicitly there to tell the FastMCP server to pay attention to this function and make it available as a tool. 
@mcp.tool()
...
def salesforce_create_opportunity(
    ...
    """
    Create a Salesforce Opportunity record for an approved AgriPilot deal.
    Automatically links to existing Account if found, otherwise creates a new one.

    Args:
        rfq_id: AgriPilot RFQ identifier e.g. AQ-2026-0412
    ...
    try:
        ...
        if account.get("found"):
            ...
            new_acct = sf.Account.create({
                "Name": dealer_company,
                "Description": f"AgriPilot auto-created — dealer for {farm_operator}",
                "Type": "Partner",
            })
        ...
        opp = sf.Opportunity.create({
            ...
            "Amount": quote_total,
            "Description": description or f"AgriPilot pipeline | Dealer: {dealer_company} | Operator: {farm_operator}",
            "LeadSource": "AgriPilot AI Pipeline",
        })
...
def docusign_send_envelope(
    ...
    """
    Send a DocuSign envelope containing the AgriPilot quotation document.
    ...
    try:
        ...
        private_key_path = os.getenv("DOCUSIGN_PRIVATE_KEY_PATH", "docusign_rsa.pem")
        ...
        envelope = EnvelopeDefinition(
            email_subject=subject or f"AgriPilot {rfq_id} — Please Sign Your Equipment Configuration",
            documents=[document],
...
def slack_notify_manager(
    ...
    try:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": f"🌾 AgriPilot Deal Alert — {rfq_id}"}
            },
            {
                "type": "section",
                "fields": [
        ...
        blocks.append({
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": f"Sent by AgriPilot AI Pipeline · {datetime.now().strftime('%Y-%m-%d %H:%M')}"}]
        })

        resp = req.post(
            "https://slack.com/api/chat.postMessage",
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json; charset=utf-8"},
            json={"channel": channel, "blocks": blocks, "text": f"AgriPilot Deal: {rfq_id}"},
            timeout=20,
        )
        data = resp.json()
...
def gmail_send_quote(
    ...
    try:
        ...
        default_token_path = _os.path.join(_os.path.dirname(__file__),"..",  "config", "token_gmail.json")
        ...
        msg["Subject"] = f"Your AgriPilot Configuration — {rfq_id} | {equipment[:50]} | {currency_symbol}{quote_total:,.0f}"

        body = textwrap.dedent(f"""
        Dear {to_name},

        Thank you for your enquiry. Please find your AgriPilot equipment configuration
        ...
        Warm regards,
        {from_name}
        Powered by AgriPilot · Autonomous Sales Engineer
        """).strip()
...
if __name__ == "__main__":
    log.info("AgriPilot MCP Server starting — 7 tools registered")
